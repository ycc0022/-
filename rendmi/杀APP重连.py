...
while i < a:
    try:
        ...
        # print("%s is %s, %d" % (fail, re.findall('电量', fail), (len(re.findall('电量', fail)))))
        if re.findall('电量', fail):
            ...
            time.sleep(2)
        elif re.findall('已连', fail):
            x = x + 1
            print('成功次数', x)
            i = i + 1
            d.app_stop('com.xiaomi.wearable')
            time.sleep(2)
        elif re.findall('失败', fail):
            y = y + 1
            print('回连失败', y)
            i = i + 1
...
print('成功总次数=', x, '失败总次数=', y, '重连失败=', z, '程序出错=', j)
os.system("pause")
print("按任意键结束")
